...
public abstract class BaseBlePlugin {
    ...
    protected List<String> getSuiteIds() {
        List<String> suiteIds = null;
        if (meterModes != null && meterModes.size() > 0) {
            suiteIds = new ArrayList<>();
            for (MeterModes m :
                    meterModes) {
                suiteIds.add(m.getSuiteId());
            }
        }
        return suiteIds == null ? null : suiteIds.size() < 0 ? null : suiteIds;
    }
    ...
    protected void monitor(Map<Integer, String> monitorData, String suiteId) {
        ...
        //数据采集方式为:Constants.RsdtMonitorListEnum.METHOD_2.code() 仪器.
        NewMonitorResult monitorResult;
        monitorResult = engine.monitorProcessByManualdrive(suiteId, monitorData, residentId, Integer.parseInt(gender),
                date, null, Constants.RsdtMonitorListEnum.METHOD_1.code());
        monitorResult.setDeviceMac(macAddress);
