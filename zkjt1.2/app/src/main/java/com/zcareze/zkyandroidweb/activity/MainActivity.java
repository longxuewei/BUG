...
public class MainActivity extends ZcarezeActivity implements OnMonitorDataReceiver, MainViewImpl {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ZigBeePlugin zigBeePlugin = ZigBeePlugin.getInstance(this, this, ZigBeePlugin.OPERATION_MODE_MONITOR_DARA);
    ...
    private void startScan() {
        BLeStatus bLeStatus = BlueToothHelper.scanBluetoothDevice(mainPresenter.scanCallBack, false);
        switch (bLeStatus) {
            case NOTOPEND:
    ...
    public void onDataReceive(MeasureResult result, String currentMode) {
        ...
        if (!(settingPresenter.checkVerecity(result.getMeasureValue())))
            return;
        NewMonitorResult newMonitorResult = mainPresenter.monitorData(this, result.getMeasureCoding(), result.getMeasureValue(),
                null, result.getSuiteId(), Constants.RsdtMonitorListEnum.METHOD_1.code());
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == AndroidCommon.OPEN_BLE_OF_LAUNCHER && resultCode == RESULT_CANCELED) {
            BlueToothHelper.blueON = false;
        }
    }
