...
public class SettingPresenter implements Javascript {
    ...
    public void stopScan() {
        BlueToothHelper.stopScan(mainViewImpl.getPresenter().scanCallBack);
    }
    ...
    private void initZGB(final String deviceType) {
        ZigBeePlugin.isBind = true;
        ZigBeePlugin.currentBindType = deviceType;
            ...
            public void onDataReceive(MeasureResult result, String currentMode) {
                ...
                if (result.getMeasureID().equals(deviceType)) {
                    if (!checkVerecity(result.getMeasureValue()))
                        return;
                    NewMonitorResult monitorResult = mainViewImpl.getPresenter().onAddDeviceDataReceive(result);
    ...
    private void initBLE(String deviceType) {
        if (!TextUtils.isEmpty(deviceType)) {
            BLeStatus status = BlueToothHelper.scanBluetoothDevice(mainViewImpl.getPresenter().scanCallBack, true);
    ...
    public int bindDevice(String jsonDeviceAddress) {
        ...
        connected.setMeterCode(deviceInfo.getDeviceType());
        connected.setSignalKind(Integer.parseInt(deviceInfo.getConnectMode()));
        connected.setIdCode(deviceInfo.getMacAddress());
        Integer result=MeterConnectedDao.getInstance(mainViewImpl.getContext()).bindDevice(connected);
        if(result==1&&deviceInfo.getConnectMode().equals("1"))
        {
