...
public class MeterConnectedDao extends BaseDao<MeterConnected> {
    ...
    public int check(String suiteid) {
        String sql = " select meter_code from meter_connected where meter_code in (select meter_modes.meter_code from meter_modes where suite_id = '" + suiteid + "' order by seq_no);";
