window.ZcarezeApp = (function(angular){
  ...
  Native.prototype.java = function(handler, params, className, errorHandler){
    ...
    this.SetupComplete = function(version){
        if(ZcarezeApp.status.status == 2){
          ...
          ZcarezeApp.rootScope.alert({
            title: '版本更新',
            template: '已安装新版本（v'+ ZcarezeApp.status.version +'），需要重启程序!',
            buttons: [
              {
                text: '重启程序',
  ...
  .controller('AppControler', ['$rootScope', '$scope', '$state', '$CLIENT','$ionicPopup', '$ionicLoading', 'rootModelView', '$modelView','$ionicTabsDelegate', '$dictionary', 'dictionaryTables' , 'SetClientService', '$ionicHistory', '$minialert', '$timeout',
    ...
    $rootScope.autoSaveMonitorDataTime = 20;
    ...
    $rootScope.initDictionary = false;
    $rootScope.loaddingDictionary = false;
    $rootScope.$dictionary = function(handler){
      $rootScope.loaddingDictionary = true;
      $dictionary.init(dictionaryTables.data, function(){
        $ionicLoading.hide();
        handler && handler();
        $rootScope.initDictionary = true;
        $rootScope.loaddingDictionary = false;
      });
    };

    $rootScope.version = property.getMetaValue('version');
      ...
      $CLIENT.load({
        ...
        if(result.data && result.data.status != 0 && window.ZcarezeBase && window.ZcarezeBase.doDownLoadRelease){
          if(+result.data.type == 0){
            ...
            ZcarezeApp.rootScope.alert({
                title: '版本（v'+ property.getAppVersion(true) +'）过期提醒！',
                template: '请下载最新版本程序（v'+ result.data.version +'），或者联系销售人员！',
                buttons: [
                  {
                    text: '在线更新',
                    ...
                    onTap: function(e) {
                      window.close();
                      ZcarezeApp.Native.java("setDownloadUrl", result.data.url);                
                    }
                  }
                ]
            });
          ...
          if(result.data.status == 2){
            ZcarezeApp.rootScope.alert({
                title: '更新提示！',
                template: '当前有重要版本（v'+ result.data.version +'）更新，请立即更新程序！',
                buttons: [
                  {
                    text: '立即更新',
                    type: 'button-energized',
            ...
            $rootScope.$minialert.show('<div>有新版本（v'+ result.data.version +'），正在准备更新...</div>');
            $timeout(function(){
            ...
            if(!$rootScope.loaddingDictionary && !$rootScope.updateApp){
              $rootScope.loaddingDictionary = true;
              $dictionary.init(dictionaryTables.data, function(){
